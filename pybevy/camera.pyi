...
class Bloom(Component):
    ...
    Bloom creates a glow effect by blurring bright parts of the image and
    compositing them back onto the original image.
    """
...
class Msaa(Component):
    """Multi-sample anti-aliasing configuration for a camera.
    ...
    Note: Web only supports Off (1 sample) or Sample4 (4 samples).
    Some advanced rendering features may require MSAA to be Off.

    Example:
        ```python
        from pybevy.camera import Msaa
...
class TemporalJitter(Component):
    ...
    Examples:
        >>> # Add temporal jitter to a camera for TAA
...
class MipBias(Component):
    ...
    Examples:
        >>> # Add mip bias to reduce blurriness with TAA
...
class OcclusionCulling(Component):
    ...
    Warning: Bevy's occlusion culling is experimental. In rare circumstances,
    meshes may be incorrectly culled (turn invisible).

    Examples:
        >>> # Enable occlusion culling on a camera
        >>> from pybevy.camera import OcclusionCulling, DepthPrepass
...
class NoAutomaticBatching(Component):
    ...
    Add this component to mesh entities when you need to manually control
    instancing or prevent the automatic batching system from grouping this
    mesh with others.
    ...
    Examples:
        >>> # Disable batching for a specific mesh
...
class NoIndirectDrawing(Component):
    ...
    When GPU preprocessing is enabled, Bevy uses indirect drawing for
    batching and instancing. This component disables that behavior for
    a specific camera, allowing manual instancing control.
    ...
    Most applications will not need this component, as indirect drawing
    generally improves performance.

    Examples:
        >>> # Disable indirect drawing for custom instancing
        >>> commands.spawn(Camera3d(), Camera(), NoIndirectDrawing())

    See Also:
        The `custom_shader_instancing` Bevy example demonstrates when this is useful.
    """
