...
def setup(
    ...
    animation_clip = asset_server.load(
        GltfAssetLabel.Animation(2).from_asset(gltf_path), asset_type=AnimationClip
    )
    ...
    commands.spawn(
        ...
        WorldAssetRoot(
            asset_server.load(GltfAssetLabel.Scene(0).from_asset(gltf_path), asset_type=WorldAsset)
        ),
    )
