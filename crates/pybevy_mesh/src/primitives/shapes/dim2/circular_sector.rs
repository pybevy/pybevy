...
#[pyclass(name = "CircularSector", module = "pybevy.math", extends = PyMeshable, eq, skip_from_py_object)]
...
#[pyconstructor("CircularSector", keyword_only(arc), conflicts((radius, half_angle), (arc)))]
#[pymethods]
impl PyCircularSector {
    ...
    pub fn new(
        ...
        if let Some(a) = arc {
            return Ok((Self(CircularSector { arc: a.try_into()? }), PyMeshable).into());
        }
        Ok((Self(CircularSector::new(radius, half_angle)), PyMeshable).into())
    }
