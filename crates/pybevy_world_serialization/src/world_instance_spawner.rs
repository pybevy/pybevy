use bevy::{
    asset::Handle,
    world_serialization::{DynamicWorld, WorldAsset, WorldInstanceSpawner},
};
...
#[pymethods]
impl PyWorldInstanceSpawner {
    pub fn spawn_dynamic(&mut self, id: &PyHandle) -> PyResult<PyInstanceId> {
        let handle: Handle<DynamicWorld> = id.try_into()?;
    ...
    pub fn spawn(&mut self, id: &PyHandle) -> PyResult<PyInstanceId> {
        let handle: Handle<WorldAsset> = id.try_into()?;
