...
use pybevy_core::{
    public_error::{
        ASSET_EVENT_NO_DEFAULT, ASSET_EVENT_READ_ONLY, ASSET_LOAD_FAILED_NO_DEFAULT,
        ASSET_LOAD_FAILED_READ_ONLY, injected_system_parameter,
    },
    registry::global_registry,
};
use pybevy_ecs::shared::{
    message_store::{
        MessageConsumeOutcome, MessageStoreError, SharedMessageCursor, new_message_cursor,
    },
...
use super::{
    ...
    python_message::{ResolvedPythonMessage, resolve_from_world},
    world::PyWorld,
};
use crate::ecs::{
...
impl MessageTypeParam {
    /// Report the held message class to the cyclic GC.
    ///
    /// A custom `Message` subclass is reachable from the annotation that names
    /// it, so the Rust-held reference closes a cycle the collector cannot see.
...
impl PyMessageWriter {
    ...
    fn traverse_python_references(&self, visit: &PyVisit<'_>) -> Result<(), PyTraverseError> {
        self.message_type.traverse(visit)?;
        if let Some(writer) = &self.python {
            visit.call(&writer.resolved.class)?;
        }
        Ok(())
    }
}
...
impl PyMessageWriter {
    ...
    fn __traverse__(&self, visit: PyVisit<'_>) -> Result<(), PyTraverseError> {
        self.traverse_python_references(&visit)
    }

    #[new]
    fn new() -> PyResult<Self> {
        Err(PyTypeError::new_err(injected_system_parameter(
            "MessageWriter",
            "writer: MessageWriter[MyMessage]",
        )))
    }
    ...
    pub fn __class_getitem__(
        ...
        let py_message_type = PyMessageType::from_annotation(key)?;
    ...
    pub fn write(&self, py: Python, message: Py<PyAny>) -> PyResult<PyMessageId> {
        ...
        match &self.message_type {
            ...
            MessageType::AssetEvent(_) => Err(PyTypeError::new_err(ASSET_EVENT_READ_ONLY)),
            MessageType::AssetLoadFailed(_) => {
                Err(PyTypeError::new_err(ASSET_LOAD_FAILED_READ_ONLY))
            }
    ...
    pub fn write_default(&self, py: Python) -> PyResult<PyMessageId> {
        match &self.message_type {
            ...
            MessageType::AssetEvent(_) => Err(PyTypeError::new_err(ASSET_EVENT_NO_DEFAULT)),
            MessageType::AssetLoadFailed(_) => {
                Err(PyTypeError::new_err(ASSET_LOAD_FAILED_NO_DEFAULT))
            }
            MessageType::Custom(_) => {
...
impl PyMessageReader {
    ...
    fn traverse_python_references(&self, visit: &PyVisit<'_>) -> Result<(), PyTraverseError> {
        if let Some(messages) = &self.messages {
            messages.message_type.traverse(visit)?;
        }
        if let Some(reader) = &self.python {
            visit.call(&reader.resolved.class)?;
        }
        Ok(())
    }
}
...
impl PyMessageReader {
    ...
    fn new() -> PyResult<Self> {
        Err(PyTypeError::new_err(injected_system_parameter(
            "MessageReader",
            "reader: MessageReader[MyMessage]",
        )))
    }
            ...
            return Ok(PyMessageReaderIter {
                ...
                python: Some(PythonMessageIterator {
                    ...
                    records: snapshot
                        .records
                        .into_iter()
                        .map(|record| PythonMessageIteratorRecord {
                            sequence: record.sequence,
                            value: record.value.as_ref().clone_ref(py),
                        })
                        .collect(),
...
struct PythonMessageIterator {
    store: crate::ecs::python_message::WeakPythonMessageStore,
    channel: pybevy_ecs::shared::message_store::MessageChannelId,
    cursor: SharedMessageCursor,
    records: Vec<PythonMessageIteratorRecord>,
    validity: ValidityFlag,
}

struct PythonMessageIteratorRecord {
    sequence: u64,
    /// An iterator-local reference, independent of the store's shared `Arc`.
    value: Py<PyAny>,
}
            ...
            loop {
                    ...
                    .map_err(store_error)?
                {
                    MessageConsumeOutcome::Consumed => {
                        return Ok(record.value.clone_ref(py));
                    }
...
#[pymethods]
impl PyMessageMutator {
    /// Report every independently owned Python reference in both handles.
    fn __traverse__(&self, visit: PyVisit<'_>) -> Result<(), PyTraverseError> {
        self.writer.traverse_python_references(&visit)?;
        self.reader.traverse_python_references(&visit)
    }
    ...
    fn new() -> PyResult<Self> {
        Err(PyTypeError::new_err(injected_system_parameter(
            "MessageMutator",
            "messages: MessageMutator[MyMessage]",
        )))
    }
