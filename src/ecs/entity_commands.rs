...
use super::{
    ...
    world_commands::{ErrorBoundary, WorldCommand},
};
...
impl PyEntityCommands {
    ...
    fn error_boundary(&self) -> PyResult<ErrorBoundary> {
        Ok(self
            .get_commands()?
            .map(PyCommands::error_boundary)
            .transpose()?
            .unwrap_or_default())
    }
    ...
    pub fn observe(&self, py: Python, observer: Bound<'_, PyAny>) -> PyResult<PyEntityCommands> {
        self.error_boundary()?.run(|| {
            if let Some(commands) = self.get_commands()?
                && commands.is_world_queue()
            {
                ObserverRegistry::validate_observer_signature(py, &observer)?;
                commands.queue_world(WorldCommand::Observe(self.id, observer.unbind()))?;
...
impl PyRelatedSpawnerCommands {
    ...
    pub fn spawn_empty(&self, py: Python) -> PyResult<PyEntityCommands> {
        self.commands_source()?.error_boundary()?.run(|| {
